...
resource "azurecaf_name" "windows" {
  ...
  name          = try(each.value.auto_name,false)?data.external.windows_name[each.key].result.name:each.value.name
...
resource "azurecaf_name" "os_disk_windows" {
  ...
  name          = try(each.value.auto_name,false)?format("%s_osdisk",data.external.windows_name[each.key].result.name):try(each.value.os_disk.name, null)
