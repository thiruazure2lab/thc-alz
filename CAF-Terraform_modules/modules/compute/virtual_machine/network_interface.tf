...
resource "azurecaf_name" "nic" {
  ...
  name          = try(each.value.auto_name,false)?try(format("nic%s_%s",data.external.windows_name["windows"].result.name,each.value.suffix),format("nic%s_%s",data.external.linux_name["linux"].result.name,each.value.suffix)):each.value.name
