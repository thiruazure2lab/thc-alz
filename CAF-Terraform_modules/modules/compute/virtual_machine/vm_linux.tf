...
resource "azurecaf_name" "linux" {
  ...
  name          = try(each.value.auto_name,false)?data.external.linux_name[each.key].result.name:each.value.name
...
resource "azurecaf_name" "linux_computer_name" {
  ...
  name          = try(each.value.auto_name,false)?data.external.linux_name[each.key].result.name:try(each.value.computer_name, each.value.name)
  resource_type = "azurerm_linux_virtual_machine"
  prefixes      = var.global_settings.prefixes
  random_length = var.global_settings.random_length
  clean_input   = true
...
resource "azurecaf_name" "os_disk_linux" {
  ...
  name          = try(each.value.auto_name,false)?format("%s_osdisk",data.external.linux_name[each.key].result.name):try(each.value.os_disk.name, null)
