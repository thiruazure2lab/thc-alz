...
resource "azurerm_app_service_slot_virtual_network_swift_connection" "vnet_config_slots" {
  for_each = {
    for key,value in try(var.slots,{}):key => value
    if try(value.settings.vnet_key,null)!=null
  }
  slot_name = each.value.name
  app_service_id = azurerm_function_app.function_app.id
  subnet_id      = try(var.vnets[try(each.value.settings.lz_key, var.client_config.landingzone_key)][each.value.settings.vnet_key].subnets[each.value.settings.subnet_key].id, null)
}
