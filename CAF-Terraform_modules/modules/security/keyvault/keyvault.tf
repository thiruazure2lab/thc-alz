...
locals {
  ...
  settings_tags = try(var.settings.tags, null) == null ? {} : try(var.settings.tags.environment, null) == null ? var.settings.tags : merge(lookup(var.settings, "tags", {}), { "environment" : var.global_settings.environment })
  tags_value    = {
    for key,value in local.settings_tags: key => value
    if key != "DeploymentDate"
  }
  combined_tags = lookup(local.settings_tags,"DeploymentDate",false)?merge(local.tags_value,{DeploymentDate=time_static.keyvault.rfc3339}):local.tags_value
}
#################################################################
