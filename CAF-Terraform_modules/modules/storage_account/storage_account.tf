################ Deployment Date Tag ############################
resource "time_static" "stg"{
}

locals {
  settings_tags = lookup(var.storage_account, "tags", null) == null ? {} : lookup(var.storage_account.tags, "environment", null) == null ? var.storage_account.tags : merge(lookup(var.storage_account, "tags", {}), { "environment" : var.global_settings.environment })
  tags_value    = {
    for key,value in local.settings_tags: key => value
    if key != "DeploymentDate"
  }
  combined_tags = lookup(local.settings_tags,"DeploymentDate",false)?merge(local.tags_value,{DeploymentDate=time_static.stg.rfc3339}):local.tags_value
}
#################################################################
