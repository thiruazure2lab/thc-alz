...
resource "azurerm_lb_backend_address_pool_address" "backend_address_pool_address" {
  ...
  virtual_network_id      = var.vnets[try(each.value.lz_key, var.client_config.landingzone_key)][each.value.vnet_key].id
...
resource "azurerm_lb_rule" "lb_rule" {
  ...
  backend_address_pool_id        = try(azurerm_lb_backend_address_pool.backend_address_pool[each.value.backend_address_pool_key].id, null)
...
resource "azurerm_network_interface_backend_address_pool_association" "vm_nic_bap_association" {
  ...
  network_interface_id    = try(each.value.network_interface_id, var.existing_resources.virtual_machines[try(each.value.lz_key,var.client_config.landingzone_key)][each.value.vm_key].nics[each.value.nic_key].id)
  ip_configuration_name   = try(each.value.ip_configuration_name, var.existing_resources.virtual_machines[try(each.value.lz_key,var.client_config.landingzone_key)][each.value.vm_key].nics[each.value.nic_key].name) # The Name of the IP Configuration within the Network Interface
  backend_address_pool_id = azurerm_lb_backend_address_pool.backend_address_pool[each.value.backend_address_pool_key].id
}
