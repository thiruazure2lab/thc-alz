...
locals {
  ...
  parameters_body = {
    ...
    hardwareFamily = {
      value = try(var.settings.sku.hardwareFamily, "Gen5")
    }
    dnsZonePartner = {
      value = try(var.primary_server_id, var.settings.dnsZonePartner ,"")
    }
    collation = {
    ...
    /* backupStorageRedundancy = {
      value = try(var.settings.backupStorageRedundancy,try(var.settings.storageAccountType, "GRS"))
    } */
  }
}